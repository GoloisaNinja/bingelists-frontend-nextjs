...
const ListCard: React.FC<ListCardProps> = (props) => {
	...
	let textOffset: string | undefined =
		props.details.type === 'tv' ? styles.yellow_offset : undefined;
	let imageSrc: string | StaticImageData =
		TMDBIMAGE_URL + props.details.posterPath;
	if (props.details.posterPath === null) {
		imageSrc = NoImage;
	}
	return (
		<Link
			href={`/media/${props.details.type}?media_id=${props.details.mediaId}`}>
			<div className={`${styles.card_container} ${colorClass}`}>
				<div className={styles.binge_card_grid}>
					<div className={styles.media_poster}>
						<Image
							src={imageSrc}
							alt={`box art for ${props.details.title}`}
							fill={true}
							sizes={
					...
					<div className={styles.item_type}>
						<p>Type: </p>
						<p className={`${styles.ratingvalue} ${textOffset}`}>
							{props.details.type}
						</p>
					</div>
					<div className={styles.primary_genre}>
						<p>Genre: </p>
						<p className={`${styles.popvalue} ${textOffset}`}>
							{props.details.primaryGenreName}
						</p>
					</div>
