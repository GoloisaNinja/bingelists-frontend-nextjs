...
const CastCrew: React.FC<CastCrewProps> = (props) => {
	...
	if (crew.length > 0) {
		...
		directorIndex = crew
			.slice(0, crew.length / 2)
			.findIndex((crew) => crew.job === 'Director');
		if (directorIndex === -1) {
			directorIndex = crew
				.slice(crew.length / 2, crew.length)
				.findIndex((crew) => crew.job === 'Director');
		}
		if (directorIndex !== -1) {
			...
			{cast.length > 0 ? (
				<div className={styles.cc_grid}>
					{castToDisplay.map((person) => (
						<CastCrewCard key={person.id} data={person} type={media_type} />
					))}
				</div>
			) : (
