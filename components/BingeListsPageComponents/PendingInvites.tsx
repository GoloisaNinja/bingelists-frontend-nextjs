...
const PendingInvites: React.FC = () => {
	...
	const actualToken =
		token?.token ||
		(typeof window !== 'undefined' ? localStorage.getItem('bingeToken') : null);

	const swrKey = actualToken
		? [`${BINGE_BASE_URL}/invites`, actualToken]
		: null;
	...
	return (
		<div className={styles.pending_container}>
			<div className={styles.invites_header}>
				<h3>
					<span className={styles.blue_span}>Sent</span> Invites
				</h3>
			...
			<div className={styles.table_container}>
				<table id={'invite_table'} className={styles.pending_table}>
					<thead>
						<tr>
							<th>List</th>
							<th>Invited</th>
					...
					<tbody>
						{data.data.map((invite: Invite) => (
							<tr key={invite._id}>
								...
								<td>
									{invite.pending ? 'pending accept' : 'no longer pending'}
								</td>
							</tr>
						))}
					</tbody>
